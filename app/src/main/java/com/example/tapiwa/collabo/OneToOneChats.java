...
public class OneToOneChats extends AppCompatActivity {
    ...
    private GenericServices checkOnline;
    private ListView ChatsListView;
    private SwipeRefreshLayout  mSwipeRefreshLayout;
    private ArrayList<Message> messagesList;
    private ChatsAdapter mAdapter;
    private int TOTAL_MESSAGES_TO_LOAD = 50;
    ...
    public static DatabaseReference mChatRoomDatabaseRootOtherUser;
    public static DatabaseReference mChatRoomNamesDatabase;
    public static DatabaseReference mOnlineStatusDbReference;
    public static final String CHAT_ROOM_CHATS_DB_PATH = "One_To_One_Chats";
    public static final String ONLINE_STATUS = "Online_Status";
    private SharedPreferences sharedPreferences;
    public String chat_room_name;
    private String uid;
    private TextView other_user_name_txtV, typing_status;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        other_user_name_txtV = (TextView) findViewById(R.id.other_user_name_toolbar_txtV);
        mSwipeRefreshLayout = (SwipeRefreshLayout) findViewById(R.id.swipeRefreshLayout);
        mDatabaseReference = FirebaseDatabase.getInstance().getReference("Users");
        onlinestatusIcon = (ImageView) findViewById(R.id.one_to_one_chat_online);
        typing_status = (TextView) findViewById(R.id.typing_status);
        ...
        mAdapter = new ChatsAdapter(OneToOneChats.this, R.layout.activity_chats_item_list, messagesList);
        input_msg = (EditText) findViewById(R.id.typeMessage);
        sharedPreferences =  PreferenceManager.getDefaultSharedPreferences(OneToOneChats.this);
        mChatRoomNamesDatabase = FirebaseDatabase.getInstance().getReference().child(GroupChatsActivity.CHAT_ROOMS_DB_REF);
        mOnlineStatusDbReference = FirebaseDatabase.getInstance().getReference(ONLINE_STATUS);
        ...
        mOnlineStatusDbReference.child(chat_room_key).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                OnlineStatus onlineStatus = dataSnapshot.getValue(OnlineStatus.class);
                typing_status.setText(onlineStatus.status);

                if(onlineStatus.status.equals("online")) {
      ...
      /*  input_msg.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                mOnlineStatusDbReference.child(user).setValue(status);
            }
        }); */
        ...
        mOnlineStatusDbReference.child(chat_room_key).keepSynced(true);
        ...
        mDatabaseReference.child(chat_room_key).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                final NewUser otherUser = dataSnapshot.getValue(NewUser.class);

                other_user_name = otherUser.getName();
                other_user_name_txtV.setText(other_user_name);

                final CircleImageView imageView = (CircleImageView) findViewById(R.id.other_user_imgV);

                Picasso.with(OneToOneChats.this)
                        .load(otherUser.thumb_image)
        ...
        mChatRoomDatabaseRoot = FirebaseDatabase
                ...
                .child(user).child(chat_room_key);

        mChatRoomDatabaseRootOtherUser = FirebaseDatabase
                .getInstance()
                .getReference()
                .child(CHAT_ROOM_CHATS_DB_PATH)
                .child(chat_room_key).child(user);

        mChatRoomDatabaseRootOtherUser.keepSynced(true);

        mChatRoomDatabaseRoot.keepSynced(true);
  ...
  public void sendMessage() {
      ...
      DatabaseReference other_message_root = mChatRoomDatabaseRootOtherUser.child(temp_key);
    ...
    /*  Map<String, Object> new_message = new HashMap<String, Object>();
      ...
      other_message_root.setValue(message);
